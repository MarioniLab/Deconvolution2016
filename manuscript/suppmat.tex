...
\end{align*}
The above relationship only holds for all cells when $\beta'_{i0}=\log(\lambda_{i0})$ and $\beta'_{i1} = 1-b$, where $\beta'_{i0}$ and $\beta'_{i1}$ are the values of the coefficients in the ``switched'' configuration.
...
In summary, a large number of assumptions are used here, but this is likely to be inevitable given that the true size factors (which would facilitate a direct comparison to the estimates) are unknown in real data.
}
