...
\revised{In practice, $E(\Theta_j)$ and $E(\Theta'_j)$ are unknown so we replace them with the observed size factor estimates $\hat\theta_j\sim\Theta_j$ and $\hat\theta'_j\sim\Theta'_j$, respectively, in the GLM fit.
...
This indicates that the estimation error of each size factor is negligible relative to the variability of the size factors across cells.
}
