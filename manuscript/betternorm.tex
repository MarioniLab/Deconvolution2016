...
Compared to size factor and TMM normalization, the deconvolution method yields a wider spread of size factor estimates (Figure~\ref{fig:real_spread}).
...
\begin{figure}[btp]
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Zeisel_NormFactors.pdf}
        \subcaption{}\label{subfig:spread_brain}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Klein_NormFactors.pdf}
...
Deconvolution yields a wider range of estimates compared to size factor and TMM normalization, consistent with the simulation results.
...
\begin{figure}[btp]
    \begin{minipage}{0.33\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Zeisel_SFvDeconv.png}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Klein_SFvDeconv.png}
    ...
    \end{minipage}
    \begin{minipage}{0.33\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Zeisel_LibvDeconv.png}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Klein_LibvDeconv.png}
        \subcaption{}\label{subfig:comp_lib}
    \end{minipage}
    \caption{
        Comparisons between the estimated size factors from the deconvolution method to those from (\subref{subfig:comp_sf}) size factor,  (\subref{subfig:comp_tmm}) TMM
...
To gauge the impact of the differences between deconvolution and size factor normalization, 
    a DE analysis was performed with DESeq2 \cite{love2014moderated} on both data sets with the different size factor estimates.
For the brain data set, the data was subsetted to contain only microglia and oligodendrocytes.
A maximum \textit{a posteriori} estimate of the NB dispersion was obtained for each gene, and a generalized linear model was fitted using a one-way layout with the two cell types.
A Wald test was performed to test for DE between cell types.
The Benjamini-Hochberg correction was applied to the $p$-values to control the false dicovery rate (FDR) at a threshold of 10\%.
This process was repeated for the inDrop data to test for DE after LIF withdrawal.

Substantial differences are present in the numbers of DE genes between the different normalization methods (Table~\ref{tab:real_de}).
In general, deconvolution results in fewer DE genes than size factor and TMM normalization.
The numbers of DE genes are also more balanced between up- and downregulated genes.

For any existing method, DE genes in one direction that are not detected by deconvolution are replaced by genes in the other direction.
...
These results indicate that different normalization schemes will have non-negligible effects on the downstream analyses.
For example, incorrect normalization will introduce spurious differences in expression for non-DE genes by failing to properly correct for cell-specific biases.
Conversely, the log-fold changes for genuine DE genes may be distorted such that detection power is decreased.

% Indicates that there is a biological effect.

\begin{table}[bt]
\caption{
    Number of DE genes detected by DESeq2 at a FDR of 10\% in each data set, using the size factor estimates from different methods for normalization.
    ...
    The number of DE genes that are shared between deconvolution and each other method is also shown.
}
\begin{center}
...
\hline
Size factor                         & 3910 & 630  & 3280 && 3000 & 3000 & 500 \\
TMM                                 & 3910 & 630  & 3280 && 3000 & 3000 & 500 \\
Library size                        & 3483 & 2000 & 1483 && 3000 & 3000 & 500 \\
Deconvolution                       & 3492 & 1556 & 1936 && 1000 & 1000 & 1000 \\
$\quad$ shared with size factor     & 2566 & 630  & 1936 && 3000 & 3000 & 500 \\
$\quad$ shared with TMM             & 2566 & 630  & 1936 && 3000 & 3000 & 500 \\
...
Substantial differences are present between the size factors from spike-in normalization and those from deconvolution, size factor and library size normalization. 
Specifically, there is no qualitative correlation between the sets of size factors from spike-in and the other normalization methods (Figure~\ref{fig:real_spike}).
...
Here, all cells are derived from the same tissue, so a non-DE majority of genes across cells is not entirely unreasonable.
...
\begin{figure}[btp]
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/SFvERCC.png}
        \subcaption{}\label{subfig:spike_sf}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/TMMvERCC.png}
        \subcaption{}\label{subfig:spike_tmm}
    \end{minipage} \\ 
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/LibvERCC.png}
        \subcaption{}\label{subfig:spike_lib}
    \end{minipage} 
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/DeconvERCC.png}
        \subcaption{}\label{subfig:spike_sum}
    \end{minipage}
    \caption{
        Comparisons between the estimated size factors from spike-in normalization to those from (\subref{subfig:spike_sf}) size factor, (\subref{subfig:spike_tmm}) TMM,
...
% Examination of DE? Maybe not a like-for-like.
