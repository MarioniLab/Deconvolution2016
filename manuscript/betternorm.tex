...
]{\inits{ATL}\fnm{Aaron TL} \snm{Lun}}
\author[
   addressref={aff2},
...
Single-cell RNA sequencing is widely used to profile gene expression in individual cells.
Analysis of this data provides biological insights at the single cell level, e.g., by identifying novel cellular subpopulations or in detecting differentially expressed or highly variable genes within populations.
However, all of these applications depend on proper normalization to eliminate cell-specific biases prior to conducting quantitative comparisons between cells.
Calculation of size factors for normalization is not straightforward for noisy single-cell data where many counts are zero.

\parttitle{Results} 
Existing normalization methods such as DESeq, TMM or library size normalization yield inaccurate size factor estimates in simulations with many zero counts and differentially expressed genes. 
We propose a novel approach where expression values are summed across pools of cells, and the summed values are used for median-based normalization.
This improves normalization accuracy by reducing the incidence of zero counts in each pool.
...
Similar behaviour is observed in real data where deconvolution yields a wider range of size factors.
The use of deconvolution also changes the number and ranking of differentially expressed and highly variable genes.

\parttitle{Conclusions} 
The deconvolution method overcomes the deficiencies of existing methods for normalization of single-cell RNA sequencing data.
We anticipate that increased normalization accuracy will improve the performance of downstream analyses and the validity of the resulting biological conclusions.
...
Two broad classes of normalization approaches are available -- those using spike-in RNA sets, and those using the counts from the profiled cellular RNA.
...
It is defined as $\lambda_{is}=\phi_{is}\lambda_{i0}$ where $\phi_{is}$ represents the DE fold change for this gene in this subpopulation, and 
\[
...
DE was introduced by setting $\phi_{is}$ to some above-unity constant $\phi_s$ for $p_s$ of the $G$ genes (i.e., upregulated) and to $0$ for the rest (downregulated, corresponding to semi-systematic zeroes).
...
Repeating the estimation of $E(r_{ik})$ for different sets of cells as $\mathcal{S}_{k}$ will generate an overdetermined system of linear equations, 
...
If cells with very small and very large values for $\theta_j T_j^{-1}$ are pooled together, the pool-based factor $r_{ik}$ is likely to be large. 
Small relative errors in estimation of large $r_{ik}$ will manifest as large relative errors for small $\theta_j T_j^{-1}$ when the linear system is solved.
...
% The only exception to the requirement for a non-DE set is if you have (a pool of) cells with expression equal to the average of many DE values.
% This would give you non-DE ratios that would (hopefully) be the median. 
% However, this seems coincidental to the extreme.
...
We favour hierarchical clustering on correlation-based distances as it avoids any circularity between normalization and clustering
...
\begin{figure}[!h]
\caption{
    \csentence{Illustration of the effect of removing stochastic zeroes (black) from the distribution of ratios across all genes, for (a) small and (b) large cells.}
