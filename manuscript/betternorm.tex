...
This avoids the bias towards unity in the simulations and results in a wider spread of size factor estimates in Figure~\ref{fig:real_spread}.

\begin{figure}[btp]
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure1a.pdf}
        \subcaption{}\label{subfig:spread_brain}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure1b.pdf}
...
The size factor estimates are mostly correlated between the different methods on real data (Figure~\ref{fig:real_comp}).
This indicates that all methods reach similar qualitative conclusions, i.e., cells with larger $\theta_j$ also have larger size factor estimates.
However, the quantitative aspect is substantially different between methods.
Deconvolution yields a wider range of estimates compared to size factor normalization, consistent with the simulation results.
Differences are also present between the estimates from deconvolution and those from library size normalization in Figure~\ref{subfig:comp_lib_brain}.
This is attributable to the likely presence of DE genes between cell types across the brain data set, which is sufficient to confound library size normalization.
In contrast, deconvolution uses a median-based approach that is robust to extreme ratios caused by DE.
The two methods are more similar for the inDrop data set (Figure~\ref{subfig:comp_lib_indrop}) where less DE is expected within the same cell type.

\begin{figure}[btp]
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure2a.png}
        \subcaption{}\label{subfig:comp_sf_brain}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure2c.png}
        \subcaption{}\label{subfig:comp_sf_indrop}
    \end{minipage} \\
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure2b.png}
        \subcaption{}\label{subfig:comp_lib_brain}
    \end{minipage}
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure2d.png}
        \subcaption{}\label{subfig:comp_lib_indrop}
    \end{minipage}
    \caption{
        Comparisons between the estimated size factors from the deconvolution method to those from size factor (\subref{subfig:comp_sf_brain}, \subref{subfig:comp_sf_indrop})
            or library size normalization (\subref{subfig:comp_lib_brain}, \subref{subfig:comp_lib_indrop}).
        This is shown for the brain (\subref{subfig:comp_sf_brain}, \subref{subfig:comp_lib_brain}) and inDrop data sets 
            (\subref{subfig:comp_sf_indrop}, \subref{subfig:comp_lib_indrop}).    
...
\begin{figure}[btp]
    \begin{minipage}{0.33\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure3a.png}
%        \includegraphics[width=\textwidth]{
        \subcaption{}\label{subfig:spike_sf}
    \end{minipage}
    \begin{minipage}{0.33\textwidth}
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure3b.png}
        \subcaption{}\label{subfig:spike_lib}
    \end{minipage} 
    \begin{minipage}{0.33\textwidth}
%        \includegraphics[width=\textwidth]{
        \includegraphics[width=\textwidth,trim=0mm 5mm 0mm 15mm,clip]{realdata/Figure3c.png}
        \subcaption{}\label{subfig:spike_sum}
    \end{minipage}
    \caption{
        Comparisons between the estimated size factors from spike-in normalization to those from (\subref{subfig:spike_sf}) size factor,
            (\subref{subfig:spike_lib}) library size normalization or (\subref{subfig:spike_sum}) the deconvolution method in the brain data set.
        Axes are on a log-scale, and the blue line represents equality between the two sets of factors.
    }
