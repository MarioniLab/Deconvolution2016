...
% In friendly cases, the extra pool sizes provide a only small decrease in the variability of the estimates.
% This is because they're mostly redundant -- the pooled size factors are calculated from the same counts, so the errors are going to be correlated. 
% In less-friendly cases, the smaller pools are necessary to mitigate errors from pooling together cells with very different size factors (e.g., poolsim.R with size <- 190L).
% Conversely, the larger pools are necessary for very low counts where the smaller pools might not be accurate with respect to the median-based estimator.
...
% This has little effect on the estimates themselves (due to redundancies above) but will give a more accurate estimate of the standard error.
