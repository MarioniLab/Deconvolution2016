...
A comparison between the size factors for different methods indicates that the estimates are mostly proportional between methods (Figure~\ref{fig:real_comp}).
This indicates that the different methods reach similar qualitative conclusions, i.e., cells with larger $\theta_j$ also have larger size factor estimates.
...
Differences are also present between the estimates from deconvolution and those from library size normalization.
This is attributable to the likely presence of DE genes across the data set, which is sufficient to confound library size normalization.
