...
\parttitle{Results} 
Existing normalization methods such as DESeq, TMM or library size normalization yield inaccurate size factor estimates in simulations with many zeroes and differentially expressed genes. 
We propose a new method for normalization where expression values are summed across pools of cells, and the summed values are used for median-based normalization.
Pool-based size factors are then deconvolved to yield cell-based factors.
This reduces the incidence of zeroes while also protecting against differential expression.
Our deconvolution approach provides accurate estimates for the size factors in analyses of simulated data.
Similar behaviour is observed in real data where deconvolution yields a wider range of size factors.
The use of deconvolution also alters the number and ranking of differentially expressed and highly variable genes.
...
We anticipate that increased normalization accuracy will improve the performance of downstream analyses and the validity of biological conclusions derived from the data.
...
For simplicity, cells in the intervening time points were not used.
...
\begin{figure}[!h]
\caption{
    \csentence{Diagram of the deconvolution method.}
