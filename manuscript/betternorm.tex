...
\revised{Let the count of a non-DE gene $i$ in cell $j$ be sampled from some distribution $Y_{ij}$ with mean $\theta_j\lambda_{i0}$, where $\theta_j$ is the cell-specific bias and $\lambda_{i0}$ is the expected transcript count (which is identical across cells for a non-DE gene).
...
Similarly, define $U_{i}$ as the mean of $Z_{ij}$ across all $N$ cells in the entire data set, which has an expectation of
\[
...
Let $R_{ik}$ denote the ratio of $V_{ik}$ to $U_{i}$ for any non-DE gene $i$.
...
Denote the realizations of $Y_{ij}$, $V_{ik}$, $U_{i}$ and $R_{ik}$ as $y_{ij}$, $v_{ik}$, $u_i$ and $r_{ik}$, respectively, where the relationships between these observed values are identical to those between their corresponding random variables.
