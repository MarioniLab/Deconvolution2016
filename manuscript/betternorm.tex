...
Define the adjusted expression of gene $i$ as $\pi_{ij} = y_{ij}T_j^{-1}$, where $T_j$ is a constant adjustment factor for cell $j$.
...
This is because it might introduce artificial differences between cells in different clusters, 
    such that the statistical rigour of downstream analyses (e.g., to detect DE between clusters) would be compromised.
However, these problems are avoided by the deconvolution method due to its use of a two-step normalization strategy.
The first normalization step removes any systematic differences between cells in each cluster, while the second normalization between pseudo-cells removes any differences between clusters.
...
This is attributable to the likely presence of DE genes between different cell types.
Any upregulation will increase the size factors for affected cells by increasing the library sizes.
...
The total number of DE genes from deconvolution is generally similar to that of the other methods (Table~\ref{tab:real_de}).
However, the number of DE genes changing in each direction is substantially different in both data sets.
Deconvolution yields a more balanced set of DE genes, with smaller differences between the numbers of up- and downregulated genes.
...
In the data sets tested here, over a thousand genes are found or lost when the deconvolution method is used instead of the existing methods.
