...
% (Precision also drops, but that's less important unless you're down to under 10 genes.)
...
% See the following: www.stat.cmu.edu/~hseltman/files/ratio.pdf, where E(V/U) ~= E(V)/E(U) - cov(V, U)/E(U)^2 + var(U)E(V)/E(U)^3
% Yes, there's a covariance in the error term, but this will drop as the variance of U drops.
% More precisely, we note that, in this setting, U=(V+X)/N where X is independent of V and represents all the other cells.
% As a result, E(V/U) ~= E(V)/E(U) - cov(V, V+X)/E(U)^2/N + var(V+X)E(V)/E(U)^3/N^2
% The error term then becomes: -var(V)/E(U)^2/N + var(V+X)E(V)/E(U)^3/N^2
%                            = 1/E(U)^3/N^2 [ var(V+X)E(V) - N var(V) E(U) ]
%                            = 1/E(U)^3/N^2 [ var(X)E(V) + var(X)E(V) - var(V)E(V) - var(V)E(X) ]
%                            = 1/E(U)^3/N^2 [ var(X)E(V) - var(V)E(V) ]
...
% Thus, E(V)=P E(Y), var(V) = P var(Y), E(X) = (N-P) E(Y) and var(X) = (N-P) var(Y).
% The error term can then be broken down into: 
%                              1/E(U)^3/N^2 [ (N-P)P E(Y)var(Y) - (N-P)P E(Y)var(Y) ] = 0
...
%
% Of course, the main motivation is still to reduce the number of zeros. 
...
% The effective value is probably lower because some zeros are semi-systematic and you need multiple non-zero counts for the median to work.
...
% This is justified by considering that the within-cluster normalization removes biases between each cell and the cluster-specific pseudo cell.
