...
This introduces large absolute errors that would manifest as very large relative errors in the size factors for smaller libraries.
By dividing by the library size, we equalize the variance of large NB-distributed counts across libraries.
Specifically, for a NB distribution with mean $\t_j\mu$ and dispersion $\varphi$, the variance of the scaled counts becomes $\mu t_j^{-1} + \varphi\mu^2$.
This is effectively constant across $j$ when $\mu$ is large.

For smaller counts, scaling will invert the Poisson mean-variance relationship and means that the smallest libraries end up dominating instead!
This leads to the second protective mechanism, i.e., the library size-sorted ring.
Small libraries are mostly pooled with other small libraries, such that no one can really dominate the variance of the pool-based size factor.

That said, the scaling can cause problems in pathological scenarios.
Recall that the median-based estimator is biased, resulting in errors in the normalization factors.
If $t_j$ is very large, the estimated _normalization factor_ will be small with a large relative error.
