...
On a more philosophical note: normalizing on library size is usually sufficient for purposes of cell type identification.
...
var(X) &= (N-P) var(Y) \; \mbox{such that} \\
...
This should be the case for pooled counts, where the sum should approach normality via the CLT.
Similarly, a NB distribution with increasing mean and decreasing dispersion will also ensure that the mean approaches the median.
...
The previous version of this algorithm pre-scaled the counts by the library size, see $t_j$ in the manuscript.
This was intended to equalize the contribution of small and large cells to each pool (and to the average reference pseudo-cell).
However, this seems to cause more problems than it's worth when the library sizes are variable _and_ there are many DE genes.
Recall that the median-based estimator is biased, resulting in errors in the normalization factors.
Upon rescaling by $t_j$, the error is amplified in the size factors.
This is most apparent in situations like below:
...
X2 <- computeSumFactors(t(t(counts)/lib.sizes), min.mean=0)

makeSFPlot(X, truth, de.cell,col=rep(c("red", "blue"), c(10, 90)))
makeSFPlot(X2*lib.sizes, truth, de.cell,col=rep(c("red", "blue"), c(10, 90)))
```

To avoid this problem, we no longer pre-scale by the library sizes prior to normalization.
Note that this only applies to the values used for pooling; the average pseudo-cell is computed using `calcAverage` with scaling.
This ensures that the average reference is not driven by a few large cells.

With hindsight, domination of each pool is less of a concern as we have already sorted by library size.
This means that large libraries are mostly pooled with other large libraries, such that no one can really dominate the final pool-based size factor.
In fact, pre-scaling didn't even solve the problem in the first place, as domination of the pool _really_ refers to the contributed variance in the ratios.
Division by the library size just inverts the Poisson mean-variance relationship and means that the smallest libraries end up dominating instead!
Perhaps there is more of a case for doing so with NB data, but the disadvantages from the issues above are too great.

## Errata for the ratios

I should also mention that $E(V_{ik})/E(U_i)$ actually represents the true size factor for pool $k$.
The per-gene expectation of $R_{ik}$ is our approximation for the true size factor, not the other way around as the text incorrectly states. 

Despite its notation, the true size factor does not actually depend on the gene as all terms related to $i$ cancel out.
This motivates the use of a robust average of $R_{ik}$ _across genes_ to estimate the true size factor.
How you compute the mean of the expectation across genes is irrelevant if the per-gene expectations are assumed to be the same (as the true size factor).
...
The main reason for sorting by library size is actually different to that described in the text.
...
Indeed, randomly scattering these cells around the ring will reduce the accuracy of all cells, large and small (see `poolsim.Rout`).
...
A related concern is that low count, high dispersion genes will have highly variable $R_{ik}$.
...
## Why is the assumption violated?
...
The clustering weakens this assumption by reducing the amount of DE _within_ each cluster.
This avoids failure of the median-based estimator for heterogeneous populations (see `moresims` for examples).
Even in relatively homogeneous populations, clustering will improve accuracy by avoiding skews in the distribution of the ratios.

Of course, one might argue that we would not need deconvolution at all if we were able to cluster cells into their types.
...
The appeal of deconvolution is that it is robust to imprecisions in the clustering (especially in high dimensions) that leave some DE genes behind.
...
This is true enough, provided that sufficient cells are still present in each cluster for pooling and stable calculation of the average pseudo-cell.

A more subtle effect is there will be more all-zero and low-abundance genes in each cluster, due to semi-systematic zeroes specific to each cell type.
...
## Do not cluster with binarized data
...
Thus, to provide further protection, I have changed the default pool sizes to go from 21, 26, 31, etc. to 101.
This reduces the chance that there is any GCD with the number of cells in the data set.
The previous default (20 to 100) would have meant that estimation precision would degrade when the number of cells is a multiple of 5.
